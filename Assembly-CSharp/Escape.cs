...
{
	private const float GOAL_RESPAWN_TIME = 5f;
	...
	private float goalRespawnTime;

	private bool isGoalRespawn;
	...
	private void OnLoadComplete()
	{
		...
		if (!MyInfoManager.Instance.GetCommonMask(MyInfoManager.COMMON_OPT.DONOT_ESCAPE_GUIDE))
		{
			EscapeGuideDialog escapeGuideDialog = (EscapeGuideDialog)DialogManager.Instance.GetDialogAlways(DialogManager.DIALOG_INDEX.ESCAPE_GUIDE);
			if (escapeGuideDialog != null && !escapeGuideDialog.DontShowThisMessageAgain)
			{
				((EscapeGuideDialog)DialogManager.Instance.Popup(DialogManager.DIALOG_INDEX.ESCAPE_GUIDE, exclusive: false))?.InitDialog();
			}
		}
	}
