...
public class BungeeMatchSituation : MonoBehaviour
{
	...
	private void OnGUI()
	{
		if (MyInfoManager.Instance.isGuiOn && on)
		{
			...
			LabelUtil.TextOut(new Vector2(killX, resultY), StringMgr.Instance.Get("SCORE"), "MiniLabel", Color.white, GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
			LabelUtil.TextOut(new Vector2(scoreX, resultY), StringMgr.Instance.Get("ROOM_TYPE_BUNGEE"), "MiniLabel", Color.white, GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
	...
	private float GridOut(int clanMark, int xp, int rank, string nickname, int kill, int assist, int death, int score, float avgPing, int status, bool isDead, float y, bool isHost)
	{
		...
		LabelUtil.TextOut(new Vector2(killX, y), score.ToString(), "MiniLabel", new Color(0.83f, 0.49f, 0.29f), GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
		LabelUtil.TextOut(new Vector2(scoreX, y), death.ToString(), "MiniLabel", Color.white, GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
		if (status == 4)
		{
