...
[Serializable]
public class MapDetailDlg : Dialog
{
	private RegMap reg;

	private Rect crdOutline = new Rect(18f, 77f, 578f, 560f);
	...
	public Vector2 crdAccusation = new Vector2(40f, 218f);

	public Vector2 crdMapWarning = new Vector2(40f, 233f);
	...
	private Rect crdBuyPoint = new Rect(35f, 600f, 21f, 22f);

	private Rect crdBuyBrick = new Rect(155f, 600f, 21f, 22f);

	private Rect crdBuyCash = new Rect(275f, 600f, 21f, 22f);

	private Vector2 crdPrice = new Vector2(33f, 670f);
	...
	{
		id = DialogManager.DIALOG_INDEX.MAP_DETAIL;
	}
	...
	{
		bool result = false;
		int num = downloadFee = ((!RegMapManager.Instance.IsDeleted(reg.Map)) ? reg.DownloadFee : 0);
		switch (buyHow)
		{
		case Good.BUY_HOW.GENERAL_POINT:
			if (BuildOption.Instance.Props.useBrickPoint)
			...
			result = (MyInfoManager.Instance.Point >= downloadFee);
			break;
		case Good.BUY_HOW.BRICK_POINT:
			result = (MyInfoManager.Instance.BrickPoint >= downloadFee);
			break;
		case Good.BUY_HOW.CASH_POINT:
			if (BuildOption.Instance.Props.useBrickPoint)
			{
				downloadFee = Mathf.FloorToInt((float)num * 0.2f);
			}
			else
			{
				downloadFee = Mathf.FloorToInt((float)num * 0.02f);
			}
			result = (MyInfoManager.Instance.Cash >= downloadFee);
			break;
		}
		return result;
	}
	...
	public override bool DoDialog()
	{
		...
		else
		{
			...
			float num = (float)GlobalVars.Instance.iconLockSlot.width;
			float num2 = (float)GlobalVars.Instance.iconLockSlot.height;
			float x = crdThumbnail.x + crdThumbnail.width / 2f - num / 2f;
			float y = crdThumbnail.y + crdThumbnail.height / 2f - num2 / 2f;
			TextureUtil.DrawTexture(new Rect(x, y, num, num2), GlobalVars.Instance.iconLockSlot, ScaleMode.StretchToFill);
		}
		...
		if (BuildOption.Instance.Props.UseAccuse && reg.IsAbuseMap())
		{
			float x2 = crdThumbnail.x + crdThumbnail.width - (float)GlobalVars.Instance.iconDeclare.width;
			TextureUtil.DrawTexture(new Rect(x2, crdThumbnail.y, (float)GlobalVars.Instance.iconDeclare.width, (float)GlobalVars.Instance.iconDeclare.height), GlobalVars.Instance.iconDeclare, ScaleMode.StretchToFill);
			LabelUtil.TextOut(crdAccusation, StringMgr.Instance.Get("REPORT_GM_MAP_REASON_PRINT_01"), "MiniLabel", Color.red, new Color(0f, 0f, 0f, 0f), TextAnchor.UpperLeft);
			LabelUtil.TextOut(crdMapWarning, StringMgr.Instance.Get("REPORT_GM_MAP_REASON_PRINT_02"), "MiniLabel", Color.red, new Color(0f, 0f, 0f, 0f), TextAnchor.UpperLeft);
		}
		...
		{
			Rect position = new Rect(viewRect.x, viewRect.y + num3, 18f, 18f);
			...
			Vector2 pos2 = new Vector2(viewRect.x + 30f, viewRect.y + num3);
			...
			float num4 = style.CalcHeight(new GUIContent(GlobalVars.Instance.snipets[i].cmt), viewRect.width - 130f);
			GUI.Label(new Rect(viewRect.x + 130f, viewRect.y + num3, viewRect.width - 130f, num4), GlobalVars.Instance.snipets[i].cmt, "MiniLabel");
			num3 = ((!(num4 <= 20f)) ? (num3 + (num4 + 4f)) : (num3 + 20f));
		}
		rectHeight = num3;
		GUI.EndScrollView();
		...
		{
			LabelUtil.TextOut(crdPrice, StringMgr.Instance.Get("NOTICE_BLOCK_MAP"), "MidLabel", Color.red, new Color(0f, 0f, 0f, 0f), TextAnchor.MiddleLeft);
		}
		else if (!reg.IsLatest)
		{
			LabelUtil.TextOut(crdPrice, StringMgr.Instance.Get("NOTICE_PREV_MAP"), "MidLabel", Color.red, new Color(0f, 0f, 0f, 0f), TextAnchor.MiddleLeft);
		}
		...
		Rect rc2 = new Rect(size.x - 377f, size.y - 44f, 176f, 34f);
		if (BuildOption.Instance.Props.UseAccuse && GlobalVars.Instance.MyButton(rc2, StringMgr.Instance.Get("REPORT_GM_TITLE_02").ToUpper(), "BtnAction"))
		{
			((AccusationMapDialog)DialogManager.Instance.Popup(DialogManager.DIALOG_INDEX.ACCUSATION_MAP, exclusive: true))?.InitDialog(reg);
		}
		Rect rc3 = new Rect(size.x - 187f, size.y - 44f, 176f, 34f);
		if (RegMapManager.Instance.IsDownloaded(reg.Map))
		{
			GUIContent content = new GUIContent(StringMgr.Instance.Get("DELETE").ToUpper(), GlobalVars.Instance.iconGarbage);
			if (GlobalVars.Instance.MyButton3(rc3, content, "BtnAction"))
			{
				CSNetManager.Instance.Sock.SendCS_DEL_DOWNLOAD_MAP_REQ(reg.Map);
			}
		}
		else
		{
			bool enabled = GUI.enabled;
			GUI.enabled = reg.IsLatest;
			if (GUI.enabled)
			{
				GUI.enabled = !reg.Blocked;
			}
			GUIContent content2 = new GUIContent(StringMgr.Instance.Get("DOWNLOAD").ToUpper(), GlobalVars.Instance.iconDisk);
			if (GlobalVars.Instance.MyButton3(rc3, content2, "BtnAction"))
			{
				if (flag)
				{
					result = true;
