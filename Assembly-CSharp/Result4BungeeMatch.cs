...
public class Result4BungeeMatch : MonoBehaviour
{
	...
	private float scoreX = 320f;

	private float killX = 460f;

	private float deathX = 540f;
	...
	{
		int score = RoomManager.Instance.RU[0].score;
		...
		LabelUtil.TextOut(crdWinnerKillDeath, score.ToString(), "Label", Color.grey, GlobalVars.txtEmptyColor, TextAnchor.MiddleRight);
	...
	private void OnGUI()
	{
		...
		LabelUtil.TextOut(new Vector2(scoreX, resultY), StringMgr.Instance.Get("SCORE"), "Label", new Color(0.83f, 0.49f, 0.29f, 1f), GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
		LabelUtil.TextOut(new Vector2(killX, resultY), "+P", "Label", Color.white, GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
		LabelUtil.TextOut(new Vector2(deathX, resultY), StringMgr.Instance.Get("ROOM_TYPE_BUNGEE"), "Label", Color.white, GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
