...
public class CTFMatchSituation : MonoBehaviour
{
	...
	private float GridOut(int clanMark, int xp, int rank, string nickname, int kill, int death, int assist, int mission, int score, float avgPing, int status, bool isDead, float y, bool isHost)
	{
				...
				if (value != null && value.Slot < 8)
				{
					...
					if (!(null == gameObject))
					{
						...
						if (!(null == component))
						{
							...
							num = GridOut(value.ClanMark, value.Xp, value.Rank, value.Nickname, value.Kill, value.Death, value.Assist, value.Mission, value.Score, peer?.PingTime ?? float.PositiveInfinity, value.Status, component.IsDead, num, value.Seq == RoomManager.Instance.Master);
						}
					}
				}
			}
				...
				if (value2 != null && value2.Slot >= 8)
				{
					...
					if (!(null == gameObject2))
					{
						...
						if (!(null == component2))
						{
							...
							num = GridOut(value2.ClanMark, value2.Xp, value2.Rank, value2.Nickname, value2.Kill, value2.Death, value2.Assist, value2.Mission, value2.Score, peer2?.PingTime ?? float.PositiveInfinity, value2.Status, component2.IsDead, num, value2.Seq == RoomManager.Instance.Master);
						}
					}
				}
			}
