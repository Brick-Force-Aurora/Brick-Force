...
[Serializable]
public class PlayerList4DefenseFrame
{
	...
	private Vector2 crdPlayerLTBlue = new Vector2(270f, 94f);

	private Vector2 crdPlayerLTRed = new Vector2(670f, 94f);

	private Vector2 offset = new Vector2(3f, 5f);

	private Rect crdVLine = new Rect(633f, 100f, 1f, 344f);
	...
	private void aPlayer(Rect rc, BrickManDesc desc, bool isRed)
	{
		...
		if (GlobalVars.Instance.MyButton(rc, string.Empty, (!isRed) ? "ButtonBlue" : "ButtonRed") && Event.current.button == 1 && desc.Seq >= 0 && desc.Seq != MyInfoManager.Instance.Seq)
		{
		...
		if (null != gameObject)
		{
			...
			if (null != camera)
			{
				GUI.Box(new Rect(rc.x + crdPortrait.x, rc.y + crdPortrait.x, crdFace.x, crdFace.y), string.Empty, (!isRed) ? "BoxPlayerBlue" : "BoxPlayerRed");
				TextureUtil.DrawTexture(new Rect(rc.x + crdPortrait.x, rc.y + crdPortrait.y, crdPortrait.width, crdPortrait.height), camera.targetTexture, ScaleMode.StretchToFill, alphaBlend: true);
			}
		}
		if (RoomManager.Instance.Master == desc.Seq)
		{
