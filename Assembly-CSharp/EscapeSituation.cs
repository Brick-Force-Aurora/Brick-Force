...
public class EscapeSituation : MonoBehaviour
{
	...
	private void OnGUI()
	{
		if (MyInfoManager.Instance.isGuiOn && on)
		{
			...
			LabelUtil.TextOut(new Vector2(killX, resultY), StringMgr.Instance.Get("ARRIVAL_COUNT"), "MiniLabel", Color.white, GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
	...
	private float GridOut(int clanMark, int xp, int rank, string nickname, int kill, int assist, int death, int score, float avgPing, int status, bool isDead, float y, bool isHost)
	{
		...
		LabelUtil.TextOut(new Vector2(killX, y), kill.ToString(), "MiniLabel", new Color(0.83f, 0.49f, 0.29f), GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
		LabelUtil.TextOut(new Vector2(scoreX, y), score.ToString(), "MiniLabel", Color.white, GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
		if (status == 4)
		{
