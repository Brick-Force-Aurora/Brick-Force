...
public class TeamMatchConfig
{
	...
	public void OnGUI()
	{
		...
		if (room != null && regMap != null)
		{
			...
			LabelUtil.TextOut(crdMode, room.GetString(Room.COLUMN.TYPE), "MiniLabel", GlobalVars.Instance.txtMainColor, GlobalVars.txtEmptyColor, TextAnchor.MiddleLeft);
			DoOption(room);
	...
	public void DoOption(Room room)
	{
		...
		LabelUtil.TextOut(new Vector2(optionRX, y), num.ToString() + StringMgr.Instance.Get("MINUTES"), "MiniLabel", GlobalVars.Instance.txtMainColor, GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
		y += diff_y;
		...
		GUI.Box(new Rect(optionRX - crdBox.x * 0.5f, y - crdBox.y * 0.5f, crdBox.x, crdBox.y), string.Empty, "BoxTextBg");
		LabelUtil.TextOut(new Vector2(optionRX, y), room.goal.ToString() + StringMgr.Instance.Get("KILL"), "MiniLabel", GlobalVars.Instance.txtMainColor, GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
		y += diff_y;
		if (isRoom)
		{
		...
		{
			LabelUtil.TextOut(new Vector2(crdOptionLT.x, y), StringMgr.Instance.Get("ROOM_SET_WANTED") + " : " + ((!room.isWanted) ? "off" : "on"), "MiniLabel", byteColor2FloatColor, GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
			y += diff_y2;
		}
	}
