...
public class Result4Defense : MonoBehaviour
{
	...
	private Vector2 crdRTK = new Vector2(904f, 56f);

	private Vector2 crdBTK = new Vector2(964f, 56f);
	...
	{
		cvtEndCode = 0;
		if (RoomManager.Instance.endCode4Red > 0)
		{
			cvtEndCode = -1;
		}
		if (RoomManager.Instance.endCode4Blue > 0)
		{
			cvtEndCode = 1;
		}
	}
	...
	{
		int coreLifeRed = DefenseManager.Instance.CoreLifeRed;
		int coreLifeBlue = DefenseManager.Instance.CoreLifeBlue;
		...
		LabelUtil.TextOut(crdTotalStuff, StringMgr.Instance.Get("NUCLEAR_LIFE"), "Label", Color.white, GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
		LabelUtil.TextOut(crdRTK, coreLifeRed.ToString(), "PointLabel", Color.white, GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
		LabelUtil.TextOut(crdBTK, coreLifeBlue.ToString(), "PointLabel", Color.white, GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
	}
