...
namespace _Emulator
{
    ...
    public sealed class UMIQuickFilter
    {
        private string Text = "";
        ...
        /// <summary>
        /// Rebuild filtered indices for a given UserMapInfo array (keeps indices into the original array).
        /// </summary>
        public void Rebuild(UserMapInfo[] umi)
        {
            UpdateTokensIfNeeded();

            if (umi == null || umi.Length == 0)
            {
                _indices = new int[0];
                return;
            }
            ...
            {
                _indices = new int[umi.Length];
                for (int i = 0; i < umi.Length; i++)
                    _indices[i] = i;
                return;
            }

            List<int> list = new List<int>(umi.Length);
            for (int i = 0; i < umi.Length; i++)
            {
                if (Passes(umi[i]))
                    list.Add(i);
        ...
        /// Returns the possibly-updated selection index into umi[].
        /// </summary>
        public int ClampSelection(UserMapInfo[] umi, int currentRegIndex)
        {
            if (umi == null || umi.Length == 0)
                return 0;
        ...
        private bool Passes(UserMapInfo m)
        {
            if (_tokens == null || _tokens.Length == 0)
                return true;
            ...
            string haystack = alias.ToLowerInvariant();
