...
public class Briefing4Bnd : MonoBehaviour
{
	...
	private void DrawCurrentChannel()
	{
		...
		if (curChannel != null)
		{
			LabelUtil.TextOut(new Vector2(400f, 17f), curChannel.Name, "MiniLabel", new Color(0.91f, 0.6f, 0f), new Color(0f, 0f, 0f, 0f), TextAnchor.UpperLeft);
		}
	}
	...
	private void OnGUI()
	{
		...
		else if (guiStep == 1)
		{
			...
			TextureUtil.DrawTexture(GlobalVars.Instance.UIScreenRect, texPopupBg, ScaleMode.StretchToFill);
			GUI.Box(crdVLine, string.Empty, "DivideLineV");
			LabelUtil.TextOut(crdBigTitle, StringMgr.Instance.Get("MY_EQUIP").ToUpper(), "BigBtnLabel", byteColor2FloatColor, new Color(0f, 0f, 0f, 0f), TextAnchor.UpperLeft);
			if (GlobalVars.Instance.MyButton(crdCloseBtn, string.Empty, "BtnClose") || (!GlobalVars.Instance.IsModalAll() && GlobalVars.Instance.IsEscapePressed()))
			{
				guiStep = 0;
		...
		else if (guiStep == 2)
		{
			...
			TextureUtil.DrawTexture(GlobalVars.Instance.UIScreenRect, texPopupBg, ScaleMode.StretchToFill);
			GUI.Box(crdVLine, string.Empty, "DivideLineV");
			LabelUtil.TextOut(crdBigTitle, StringMgr.Instance.Get("SHOPPING").ToUpper(), "BigBtnLabel", byteColor2FloatColor2, new Color(0f, 0f, 0f, 0f), TextAnchor.UpperLeft);
