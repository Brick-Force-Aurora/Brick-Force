...
public class Result4Escape : MonoBehaviour
{
	...
	private float GridOut(ResultUnit ru, float y)
	{
		...
		LabelUtil.TextOut(new Vector2(killX, y), ru.kill.ToString(), "Label", new Color(0.83f, 0.49f, 0.29f), GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
	...
	private void OnGUI()
	{
		...
		LabelUtil.TextOut(new Vector2(killX, resultY), StringMgr.Instance.Get("ARRIVAL_COUNT"), "Label", new Color(1f, 0.66f, 0f, 1f), GlobalVars.txtEmptyColor, TextAnchor.MiddleCenter);
